...
azd hooks run postprovision   # see the caveat below - publishes the Foundry agent
```

> **Caveat, measured not assumed (azd 1.27.1 and 1.31.0, Aug 2026).** `azd up`,
> `azd provision` and `azd deploy` did **not** execute the hooks declared in
> [azure.yaml](../azure.yaml), on either version, as project **or** service hooks.
> Verified by side effect rather than by log text: the Foundry agent gained no new
> version across four separate runs. `azd hooks run <name>` resolves and executes
> them correctly from the same file, so the declaration is right and azd simply is
> not firing them here. Until that changes, run `azd hooks run postprovision` after
> a deploy, or the hosted agent keeps whatever instructions it was created with.
>
> `AUTH_SECRET` is unaffected in practice: it persists in the azd environment once
> set, and `@minLength(32)` makes a missing one fail deployment with a clear message
> rather than silently shipping an app nobody can log into. On a genuinely fresh
> environment, run `azd hooks run preprovision` first (or set it yourself).

`AUTH_SECRET` (the Auth.js session key) is generated by
[scripts/preprovision.sh](../scripts/preprovision.sh) and stored in the azd
environment on first run - it used to be a manual prerequisite, and skipping it
...
azd env set ENABLE_OTP_LOGIN true        # sign-in by emailed code - needs a CUSTOM mail domain
