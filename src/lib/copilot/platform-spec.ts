...
export function platformSpec(tools: ToolSummary[]) {
  return {
    ...
    sections: [
      ...
      { route: "/dashboard/pipeline/{id}", name: "Lead detail", permission: "lead:read", shows: "Priority, quadrant, value and expected value; next move and pace; the whole client relationship (every deal with that client, lifetime and repeat value) and its proposals; the score breakdown; notes; outreach history.", actions: "Edit every field - quick stage transitions - record a proposal - link or unlink a company - compose outreach - delete (needs lead:delete)." },
      ...
      { route: "/dashboard/quality", name: "Data quality", permission: "quality:read", shows: "Possible duplicate clients and the merge control. The import audit trail plus live checks; rows whose imported outcome contradicts their stage.", actions: "Confirm the stage is right to close a conflict (needs lead:update)." },
