...
export const TOUR_STEPS: TourStep[] = [
  ...
  },
  {
    ...
    body: "Your full, editable lead list. This is the Brands Operative tab, live.",
  ...
  },
  {
    ...
    sheet: "The badges under the name are the controls - stage, priority and industry are dropdowns you change in place, and the stage one only offers the moves the workflow allows. The edit dialog holds everything else: owner, contacts, dates, commercial value and the four 0-5 judgements behind the score. Deleting lives at the bottom of it, behind a confirm. It is permanent and needs the delete permission.",
    isNew: true,
    placement: "left",
    optional: true,
  },
  {
    ...
    route: "/dashboard/pipeline",
    selector: '[data-tour="pipe-edit"]',
    title: "Notes that keep what was already there",
    body: "A lead's notes are two things now. The initial note is what it IS - how it came in, who introduced it, what to be careful of - written once when the lead opens, and it is what exports and reminder emails lead with. Everything you learn afterwards is added below it instead, with your name and the date on it. Nothing is ever replaced by whoever saves next, which is the whole reason the old single box was not worth reading: it quietly became whatever the last person to edit it happened to care about.",
    sheet: "There is no edit and no delete on the thread, on purpose - a record that can be rewritten is back to having no memory. A correction is added like anything else. Search reads the thread too, so a phrase somebody wrote months ago still finds the lead.",
    isNew: true,
    placement: "left",
    optional: true,
  },
  ...
  },
  {
    ...
    route: "/dashboard/pipeline",
    selector: '[data-tour="nav"]',
    title: "The client behind the lead",
    body: "A client is no longer one row. Open any lead and the whole relationship is on the same page: every deal ever run with Fastweb, what it has earned in total, and how much of that was repeat business after the first win. Start the next deal for that client from there. Records that look like the same client twice are listed on Data Quality, where you can fold one into the other.",
    sheet: "Projected from Brands Operative on every read, so a client can never fall out of step with an edit to one of its deals.",
    isNew: true,
    placement: "bottom",
  },
  {
  ...
  },
  {
    ...
    selector: '[data-tour="nav"]',
    title: "Data quality",
    body: "Keeps the pipeline clean - flags missing fields, unscored leads and stale contacts so nothing slips through. It opens with a count per section that jumps straight to it, so you can see from the top whether there is anything to do. Rows whose imported outcome contradicts their stage get their own review card, and you can close one by confirming the stage is the right answer. That figure came from the original spreadsheet's outcome column and had no control anywhere, so a lead you correctly marked “Lost” used to stay flagged forever. Clients that look like the same client twice are reviewed here too, and folding one into the other is a button.",
  ...
  },
  {
    ...
    title: "Ask anything",
    body: "Start with a suggestion or ask your own. The starters changed in v1.2: “What should I do today?”, “What's at risk of going cold?” and “What am I allowed to do?” now sit alongside “Summarise the pipeline”.",
    isNew: true,
    placement: "right",
    optional: true,
  },
  {
    ...
    body: "It returns a single ranked list of every lead that needs a human - follow-ups you have missed, clients nobody has chased, deals nobody has triaged, deals gone quiet - ordered by what is at stake rather than by date, so a large deal two days late outranks a small one two weeks late. Each lead appears once, under its worst reason, and each row carries the action that clears it.",
    isNew: true,
    placement: "right",
    optional: true,
  },
  {
    ...
    body: "Toggle “Think deeply” for tougher questions - the Copilot spends more reasoning effort on the answer and shows its step-by-step thinking. In v1.2 this switch actually does something: it raises the effort sent with the request rather than pretending to swap models.",
    isNew: true,
    placement: "top",
  },
  {
    ...
    body: "Attach a PDF, Word doc, deck or spreadsheet and ask about it - the Copilot reads it and answers with citations.",
    placement: "top",
    optional: true,
  },
  {
  ...
  },
  {
    ...
    title: "It can act, too - 49 tools now",
    body: "It went from 14 tools to 49, and the gap it closed was everything the screens could do and the chat could not. It can add a lead, edit its fields, set its commercial value, move it through the stages, record or remove a proposal, merge two companies, link or unlink a deal, complete or snooze a follow-up, set strategic value, reassign an owner, add to a lead's notes and read the whole thread back, cancel a draft, send outreach and delete a lead. It can also set you a reminder - now or for a time, in the app and by email, carrying the lead's whole CRM state and blocking the time in your calendar if you ask. Plus data quality, deal pace, budget accuracy, the audit trail for one lead, dormant clients worth revisiting and the money going cold. Ask it how anything works and it reads the scoring model and the product manual back to you out of the code itself.",
    sheet: "Every tool runs as you and checks the same permission the screens check, so the chat is never a way round the rules - if you cannot delete a lead on screen, asking nicely will not do it either. Writes are logged as yours “via copilot”, and anything irreversible - deleting, merging, sending - arrives as a button you press, never fired off because a sentence sounded like a request. It also asks before it acts: give it half a request and it comes back with the missing questions in one go, tells you what leaving each blank would cost, and goes ahead the moment you say it is fine as it stands. Ask it what to put and it reasons from comparable deals - the median value for that segment, who works it, how long those deals really took - and searches the web for what the pipeline cannot know.",
    isNew: true,
    placement: "top",
  },
  {
  ...
  },
  {
    ...
    body: "v1.2 renamed the pipeline stages to Seed, Qualify lead, Shape proposal, Closed deal, Recurring and Lost, and the priority labels to High, Medium and Low. The client relationship moved onto the lead page, so a company is no longer a separate screen saying the same things. Stage, priority and industry are editable straight from the badges. Notes keep what was already there instead of replacing it. On top of the two-axis scoring model, pipeline health and granular permissions, the Copilot can now do 49 things instead of 14. Replay this tour anytime from the “?” button in the top bar.",
    placement: "center",
  },
];
